...
export default function App() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const message = await reviewApi.sendMessage(sessionId, content, controller.signal); setMessages((items) => [...items.filter((item) => item.id !== optimistic.id), optimistic, message]); setDraft(''); setLocalActivities((items) => items.map((item) => item.status === 'active' ? { ...item, status: 'completed', label: '消息已处理' } : item)); setPhase(snapshot?.results.length ? 'review' : 'confirm');
