...
if rg -n "$secret_pattern" \
  --glob '!package-lock.json' --glob '!bun.lock' --glob '!node_modules/**' \
  --glob '!dist/**' --glob '!.git/**' .; then
  echo 'repository secret scan failed' >&2
  exit 1
fi

archive=$(scripts/build-submission-zip.sh)
manifest=$(mktemp)
unpacked=$(mktemp -d)
trap 'rm -f "$manifest"; rm -rf "$unpacked"' EXIT
tar -tzf "$archive" >"$manifest"
tar -xzf "$archive" -C "$unpacked"

if rg -n '(^|/)(\.env$|.*\.db$|.*\.sqlite|.*\.wal$|.*\.shm$|node_modules|dist/|build/|\.git/|.*\.log$|.*\.bak$)' "$manifest"; then
