...
export default function App() {
  ...
  async function bootstrap() {
    try {
      ...
      setUser(me.user);
      if (me.mustChangePassword) setS("change-password");
      else setS("home");
    } catch (e) {
  ...
  async function handleLogout() {
    try {
      ...
      /* ignore — local logout still applies */
    }
    setUser(null);
    setR(null);
    setP(0);
  ...
  if (s === "login")
    return (
      <div className="app">
        ...
        <main>
          <LoginScreen bootError={bootError} onSuccess={handleLoginSuccess} />
        </main>
      </div>
    );
  if (s === "change-password" && user)
    return (
      <div className="app">
        ...
        <main>
          <Home
            ...
            go={async (e) => {
              ...
              try {
                const created = await createReview(v);
                const result = await pollResult(created.id, created.input);
                setR(result);
              } catch (err) {
                setR({
                  id: `err-${Date.now()}`,
...
function Header({
  ...
  return (
    <header>
      <b>
        <i>A</i>AIME <small>DECISION REVIEW</small>
      </b>
      <div className="headerRight">
        ...
        {user ? (
          <span className="user">
            <b>● {user.username}</b>　{user.role === "admin" ? "管理员" : "用户"}
          </span>
        ) : null}
        {user ? (
...
function LoginScreen({
  ...
  onSuccess: (u: PublicUser, m: boolean) => void;
}) {
    ...
    <section className="authShell">
      <form className="authCard card form" onSubmit={submit}>
        <div className="cardhead">
          <span>
            STEP 00
            <h2>{mustChange ? "首次登录需修改密码" : "修改密码"}</h2>
          </span>
          <span>✦</span>
        </div>
...
function AdminScreen({ onBack }: { onBack: () => void }) {
  ...
  async function handleToggle(u: PublicUser) {
    setBusyId(u.id);
    try {
      if (u.enabled) await adminUsers.disable(u.id);
      else await adminUsers.enable(u.id);
      await refresh();
    } catch (e) {
      if (e instanceof ApiError) {
    ...
    <section className="admin">
      ...
      <div className="adminGrid">
        <form className="card form" onSubmit={handleCreate}>
          ...
          <button className="primary" disabled={creating || !newName.trim()}>
            {creating ? "创建中…" : "创建并生成初始密码"}
            <ArrowRight size={16} />
          </button>
          {temp && (
            <div className="tempCard">
              ...
              <div className="tempRow">
                <code>{temp.username}</code>
                <b>{temp.temporaryPassword}</b>
              </div>
        ...
        <div className="card adminList">
          <div className="cardhead">
            ...
            <span>{users ? `${users.length} 人` : "加载中…"}</span>
          </div>
          ...
          {users &&
            users.map((u) => (
              <div
                key={u.id}
                className={"userRow" + (u.enabled ? "" : " disabled")}
              >
                <div>
                  <b>{u.username}</b>
...
function Home({
  ...
  v: Input;
  setV: React.Dispatch<React.SetStateAction<Input>>;
  go: (e: React.FormEvent) => void;
}) {
    ...
    <section className="home">
      <div className="intro">
        <label className="pill">
          <Sparkles size={13} /> Decision Replay
        </label>
        <h1>
          把一次投资，
          <br />
        ...
        <p>
          重建决策当下的证据时间线，分开事实与结果，让每一次交易都沉淀为可复用的判断力。
        </p>
        <div className="principle">
          <strong>T0</strong>
          <span>
            <b>时间边界优先</b>
            <small>只用决策发生前能获得的信息评估判断质量。</small>
          </span>
        </div>
      </div>
      <form className="card form" onSubmit={go}>
        ...
        <div className="row">
          <Field t="标的代码 / 名称">
            <input
              required
              value={v.symbol}
              onChange={(e) => u("symbol", e.target.value)}
          ...
          <Field t="市场">
            <select
              value={v.market}
              onChange={(e) => u("market", e.target.value)}
            >
              <option>A股</option>
              <option>港股</option>
              <option>美股</option>
        ...
        <div className="row">
          ...
          <Field t="成交时间">
            <input
              ...
              type="datetime-local"
              value={v.executedAt}
              onChange={(e) => u("executedAt", e.target.value)}
            />
          </Field>
        </div>
        <div className="row">
        ...
        <Field t="补充笔记（可选）">
          <textarea
            ...
            placeholder="仓位计划、止盈止损、当时的犹豫…"
          />
        </Field>
        <button className="primary">
          开始复盘 <ArrowRight size={16} />
        </button>
        <small className="safe">
          <ShieldCheck size={13} />
          你的输入仅用于本次复盘，不会写入公开日志。
        </small>
      </form>
    </section>
  );
...
function Running({ p }: { p: number }) {
  return (
    <section className="card running">
      <div className="runicon">◌</div>
      <span>REVIEW RUNNING</span>
      <h1>正在重建这笔决策</h1>
      <p>把决策时点的证据，与之后发生的结果严格分开。</p>
      <div className="bar">
        <i style={{ width: p * 20 + "%" }} />
      </div>
      {stages.map((x, i) => (
        <div className={"stage " + (i < p ? "done" : "")} key={x}>
          <b>{i < p ? <Check size={12} /> : i + 1}</b>
          {x}
          <small>{i < p ? "完成" : i === p ? "分析中…" : "等待"}</small>
        </div>
      ))}
      <div className="safe">◈ 不展示模型思考过程，仅呈现可核验的证据与结论。</div>
    </section>
  );
}

function Result({ r, reset }: { r: ReviewResult; reset: () => void }) {
  const d = r.result.decision;
  const ex = (r.result.exAnteEvidence || []).map((e) => e.title);
  const post = (r.result.exPostEvidence || []).map((e) => e.title);
  return (
    <section className="result">
      <div className="resulttop">
        <div>
          <label className="pill ok">
            <Check size={13} /> REVIEW COMPLETE
          </label>
          <h1>
            {d.symbol} <span>· {d.action === "buy" ? "买入" : "卖出"}复盘</span>
          </h1>
          <p>
            {d.executedAt.replace("T", " ")} · 决策时点 {d.T0}
          </p>
        </div>
      ...
      <div className="card summary">
        ...
        <div>
          ...
          <p>{r.result.decisionQuality.reasoning}</p>
        </div>
        <strong>
          {r.result.decisionQuality.rating}
          <small>判断质量</small>
        </strong>
      </div>
      <div className="t0">
        ...
        <strong>时间边界</strong>
        <span>
          左侧只包含当时可知信息；右侧是事后发生的信息，不能用于评价当时的判断。
        </span>
      </div>
      ...
      <div className="lower">
        <div className="card block">
          ...
          <h3>归因</h3>
          {(r.result.attribution || []).map((a, i) => (
            <div className="tag" key={i}>
              <b>{a.status}</b>
              <span>{a.claim}</span>
            </div>
          ))}
          {(r.result.attribution || []).length === 0 && (
            <p className="muted">无归因项。</p>
          )}
        </div>
        <div className="card block">
          <label>OUTCOME VS QUALITY</label>
          <h3>结果不等于质量</h3>
          <p className="muted">{r.result.outcome.summary}</p>
          <p className="muted">{r.result.outcome.note}</p>
        </div>
      </div>
      <div className="card lessons">
        <label>NEXT TIME</label>
        <h3>Lessons & Checklist</h3>
        {(r.result.lessons || []).map((x, i) => (
        ...
        {(r.result.nextChecklist || []).map((x, i) => (
          <div className="lesson" key={"c" + i}>
            <b>0{i + 1 + (r.result.lessons || []).length}</b>
            <span>
              <strong>{x}</strong>
            </span>
          </div>
      ...
      <p className="cite">
        ⓘ 证据引用：{adapterKind} adapter ·{" "}
        {(r.result.citations || []).length} 条 · 工具状态{" "}
        {(r.result.toolStatuses || []).length} 条
      </p>
    </section>
  );
}
