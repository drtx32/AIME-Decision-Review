...
export type SessionMessage = { id: string; sessionId: string; userId: string; role: 'user' | 'assistant' | 'status' | 'error'; content: string; createdAt: string; state?: 'accepted' | 'extracting' | 'needs_input' | 'failed' | 'completed' | 'cancelled'; errorMessage?: string | null };
export type ActivityEvent = { id: string; type: 'lifecycle_status' | 'reasoning_summary' | 'tool_started' | 'tool_updated' | 'tool_completed' | 'evidence_retrieved' | 'terminal_status'; label: string; purpose?: string; status: 'active' | 'completed' | 'degraded' | 'error' | 'cancelled'; provider?: string; sourceCount?: number; startedAt?: string; completedAt?: string };
...
export type SessionSnapshot = { session: { id: string; title: string; scope: string; status: string }; decisions: SessionDecision[]; messages: SessionMessage[]; memories: LearningMemory[]; activities?: ActivityEvent[]; results: Array<{ decisionId: string; reviewId: string; status: string; result: any }> };
