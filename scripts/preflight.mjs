...
function walk(rel) {
  ...
  for (const name of readdirSync(abs)) {
    ...
    // TypeScript project-build metadata is a disposable build artefact. It is
    // forbidden from submissions, but its presence after `npm run build`
    // must not make the preflight fail before the archive filter can exclude
    // it.
