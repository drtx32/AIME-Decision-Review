...
## Submission skeleton — 2026-09-23 (Local CC)

**AI/tool used**
- Local CC (this turn) on branch `agent/local-cc/705524d76999`

**Task**
- ELI-340: build a reproducible submission skeleton — manifest, deployment
  evidence template, AI usage / validation record (separating fixture from real
  validation), license inventory, test notes, a preflight script that
  verifies required artefacts, rejects secret-like files, records git
  SHA / dirty state, and can build a deterministic submission ZIP — plus
  bun:test coverage of the preflight and an entry in the Product CI
  workflow.

**Output**
- `submission/MANIFEST.md` — required artefacts (§1), forbidden paths
  and globs (§2.1), forbidden content patterns (§2.2), excluded build
  artefacts (§2.3), optional artefacts (§3), final ZIP naming convention
  (§4), determinism guarantees (§5).
- `submission/README.md` — how to use the skeleton, what it deliberately
  does not do, status note.
- `submission/DEPLOYMENT_EVIDENCE.template.md` — fill-in template for the
  URL, git SHA, image tags, smoke-test evidence, and rollback snapshot.
- `submission/LICENSE_INVENTORY.md` — direct dependencies (frontend,
  backend, container base images), inspirations / references (Hermes
  WebUI, ECharts, OpenAI Agents SDK, Hono, Bun) with explicit
  inspiration-vs-copy distinctions, things deliberately NOT vendored,
  attribution template.
- `submission/TEST_NOTES.md` — status legend, per-`TEST_PLAN.md` case
  status table, known boundaries (mock LLM, mock MCP, SQLite per-process,
  bounded reflection, bootstrap admin fail-fast, mock frontend fallback,
  auth required, identity contract, Chinese-first UI).
- `submission/AI_VALIDATION_RECORD.md` — submission-facing summary that
  splits every claim into ✅ Fixture (mock LLM/MCP) vs ⛔ Real (unresolved),
  each pointing back to a precise entry in the canonical
  `docs/AI_VALIDATION.md`.
- `scripts/preflight.mjs` — self-contained Node ESM script (no third-party
  deps, runs on Node ≥ 18 or Bun ≥ 1.1) that checks required artefacts,
  forbids forbidden paths / globs, scans every tracked file's first 256KB
  for credential patterns with placeholder-aware lookaheads, records git
  SHA + dirty state to `submission/BUILD_INFO.txt`, optionally builds a
  deterministic store-mode ZIP (pure-JS encoder, no `zip` binary
  required) or a deterministic tar.gz via the system `tar`. Module-level
  `isMain` guard keeps it side-effect-free when imported for tests.
- `scripts/preflight.sh` — POSIX wrapper that picks `node` or `bun` and
  delegates to `preflight.mjs`, so the same script works on the
  root-level Makefile / CI shell and on Git Bash on Windows.
- `tests/preflight/preflight.test.ts` — bun:test coverage (23 tests / 92
  expect() calls) exercising the path classifier, every secret-content
  pattern against real and placeholder fixtures, the secret-scan
  exemption list, the deterministic ZIP encoder (byte equality across
  reorderings, forbidden-path exclusion).
- `submission/BUILD_INFO.txt` — auto-generated by the preflight on every
  run (gitignored so it does not pollute history); included in any
  deterministic ZIP the script builds.
- `.gitignore` — `submission/BUILD_INFO.txt` added so the auto-generated
  build metadata is regenerated, not tracked.
- `.github/workflows/ci.yml` — added a `preflight` job as the dependency
  for `frontend` and `backend`, so secret / forbidden-path / required-
  artefact checks fail CI before any build step runs.
- `package.json` (root) — added `preflight`, `preflight:zip`,
  `preflight:strict`, `preflight:test` scripts.
- `README.md` — new "Submission skeleton" section with the preflight
  commands, an architecture summary, and a test-command recap.

**Validation**
- `node scripts/preflight.mjs` → exit 0, 0 errors, 1 warning
  (`working tree dirty` — the new untracked files of this PR).
- `cd apps/api && bun test ../../tests/preflight/` → 23 pass / 0 fail,
  92 expect() calls.
- `node scripts/preflight.mjs --zip /tmp/test.zip` → 428 018-byte
  deterministic ZIP, sorted entries, forbidden paths excluded.
- `node scripts/preflight.mjs --json` → well-formed summary including
  commit SHA, dirty state, finding list.
- `bash scripts/preflight.sh` → delegates to `node`, exit 0.
- Secret scan on the new `submission/` directory: zero populated
  credentials. The `Authorization: Bearer` / `sk-*` /
  `LLM_API_KEY=` / `INITIAL_ADMIN_PASSWORD=` literals in the new files
  are documentation prose or test fixtures, all matched by the negative
  lookaheads / lookbehinds in `FORBIDDEN_CONTENT` and exempted by
  `SECRET_SCAN_EXEMPT`.

**Human corrections**
- Originally anchored the secret-content patterns to `^KEY=…$/m`; that
  rejected `export const token = LLM_API_KEY=sk-live-…` as a non-match
  and failed the inline-leak test. Switched to a `(?<![A-Za-z0-9_])`
  lookbehind to allow inline code-path leaks while still requiring a
  non-identifier boundary and the placeholder-aware lookahead. Verified
  with both the inline-leak fixture and the existing "matches `KEY=` but
  ignores placeholders" cases.
- Originally required an exact-match check for FORBIDDEN_PATHS (so
  `id_rsa` only matched the top-level `id_rsa` file). Added a basename
  match so `~/.ssh/id_rsa`, `.ssh/id_rsa`, etc. are also rejected, which
  matches the manifest's intent ("id_rsa" forbidden, not "id_rsa only at
  the repo root"). Also broadened FORBIDDEN_DIRS to a path-segment check
  so `apps/api/.multica/x` is rejected.
- Moved `BUILD_INFO.txt` write to happen before the ZIP build and added
  it to the in-memory `files` list, so the build metadata ships in the
  deterministic archive.
- Did not deploy. Did not modify `src/App.tsx`, `src/api.ts`,
  `src/auth-api.ts`, or any file under `apps/api/src/`.

**Residual risk / unresolved**
- Real LLM (MiniMax) and real Fuyao / iFinD MCP calls remain
  unvalidated; `submission/AI_VALIDATION_RECORD.md` and
  `docs/AI_VALIDATION.md` both pin this as ⛔.
- Real production deployment (URL, container image tags, smoke-test
  evidence) is not part of this PR; it will populate
  `submission/DEPLOYMENT_EVIDENCE.md` from
  `submission/DEPLOYMENT_EVIDENCE.template.md` once a real run exists.
- The submission ZIP uses store mode (no compression); for typical
  text-heavy submission archives the size delta is small but not zero.
  Compression can be added later without breaking the determinism
  contract (the CRCs and file order are independent of the compression
  mode).
- The preflight scans only the first 256 KB of every file; pathological
  leaks past that offset would be missed. This is acceptable for the
  submission skeleton (real secrets do not hide past the first 256 KB of
  any source file) and is documented in `scanFileContent`.
